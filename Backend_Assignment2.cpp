...
{
    vis[u] = 1;
    if(sts.find(u)!=sts.end())
        arr[indx] = dis[u];
    else
        arr[indx] = -1000000;
    //cout<<u<<" "<<arr[indx]<<endl;
    Euler[indx++] = u;
    n = max(n,indx);
    int st = indx;
    for (auto it : adj[u]) {
...
void edfs(int u, int &indx){
    vis[u] = 1;
    indx++;
    int st = indx;
    int mxd = getSum(n, 0, n-1);
    //cout<<u<<mxd<<endl;
    if(mxd<=x)
        ans++;
    for (auto it : adj[u]) {
        if (!vis[it]) {
            int en = mpe[{u,it}];
            updateRange(n, st,en, -1);
            if(st-1>=0){
                updateRange(n,0,st-1,1);
            }
            if(indx+1<n){
                updateRange(n,en+1,n-1,1);
            }
            edfs(it, indx);
            updateRange(n, st,en, +1);
            if(st-1>=0){
